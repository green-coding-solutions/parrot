#!/usr/bin/env bash
# Install Neovim and the xterm it runs in, from Ubuntu 24.04.
#
...
#
# NOTHING IS CONFIGURED.  No init.lua or init.vim is written: Neovim runs on its
# own defaults, which since 0.9 already include syntax highlighting, a status
# line and sensible search behaviour without any config at all.
set -euo pipefail

NVIM_VERSION='0.9.5-6ubuntu2'
XTERM_VERSION='390-1ubuntu3'

log() { printf '[install-neovim] %s\n' "$*"; }
...
log "installing xterm ${XTERM_VERSION} and neovim ${NVIM_VERSION}"
...
cat > /usr/local/bin/nvim-term <<'WRAPPER'
#!/bin/sh
exec xterm \
...
WRAPPER
chmod +x /usr/local/bin/nvim-term
