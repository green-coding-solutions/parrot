#!/usr/bin/env bash
# Install Vim and the xterm it runs in, from Ubuntu 24.04.
#
...
log "installing xterm ${XTERM_VERSION} and vim ${VIM_VERSION}"
...
cat > /usr/local/bin/vim-term <<'WRAPPER'
#!/bin/sh
exec xterm \
...
WRAPPER
chmod +x /usr/local/bin/vim-term

log "installed: $(vim --version | head -n1)"
