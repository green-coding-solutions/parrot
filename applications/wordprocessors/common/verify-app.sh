...
if [[ "${1:-}" == "--normalized" ]]; then
    VARIANT="-normalized"
    shift
    export SCENARIO_OVERRIDE="${REPO}/applications/wordprocessors/${APP}/usage_scenario_normalized.yml"
fi

R=/tmp/repo/applications/wordprocessors
...
bash "${REPO}/applications/wordprocessors/common/setup-container.sh" "$APP" >/dev/null
...
md5sum applications/wordprocessors/${APP}/${APP}-check-*.png 2>/dev/null \
...
echo
echo "=== ground truth ==="
# The expected PDF page count is per-app: the apps paginate the shipped document
# differently and the run adds exactly two pages. Apps that differ from the
# default of 100 carry the number beside their scenario.
PDF_PAGES=100
...
bash "${REPO}/applications/wordprocessors/common/check-result.sh" window-container "$PDF_PAGES"
