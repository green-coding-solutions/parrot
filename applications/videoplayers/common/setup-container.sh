...
#
# Adapted from applications/terminals/common/setup-container.sh: the logic is
# group-agnostic - read the scenario, run its setup-commands through the same
# argv GMT would build - and only the group path differs.
#
...
SCENARIO="${SCENARIO_OVERRIDE:-${REPO}/applications/videoplayers/${APP}/usage_scenario.yml}"
...
# The `environment:` block is read out of the scenario for exactly the same
# reason, and in this group it is load-bearing rather than cosmetic. GMT passes
# those variables with `docker run -e`; a container built here without them is a
# DIFFERENT container, and the difference is not subtle.
#
# Found the hard way on xterm: without LANG the emulator starts in the C locale
# and draws every UTF-8 box-drawing and CJK glyph as Latin-1 mojibake - `â` where
# `│` belongs. The shell profile's LANG does not save it, because an emulator
# decides its UTF-8 mode from ITS OWN environment at startup, before the shell
# inside it exists. So a recording made in a hand-built container would have been
# a recording of mojibake, replayed against a correctly-drawn benchmark, and
# every checkpoint from block 8 on would have failed for a reason nothing in the
# macro pointed at.
mapfile -t ENV_ARGS < <(python3 - "$SCENARIO" <<'PY'
import re, sys
text = open(sys.argv[1]).read()
m = re.search(r'^\s*environment:\s*\n((?:\s*(?:#.*|[A-Za-z_][A-Za-z0-9_]*:\s*.+)\n)+)', text, re.M)
if m:
    for line in m.group(1).splitlines():
        line = line.strip()
        if not line or line.startswith('#'):
            continue
        k, v = line.split(':', 1)
        print('-e'); print(f'{k.strip()}={v.strip()}')
PY
)
# RESOLUTION comes from the scenario like everything else, but it decides the X
# screen size and a container without it is not worth debugging, so fall back
# rather than start a 1x1 display.
printf '%s\n' "${ENV_ARGS[@]}" | grep -q '^RESOLUTION=' ||
    ENV_ARGS+=(-e RESOLUTION=1440x900)
[[ ${#ENV_ARGS[@]} -gt 0 ]] && echo "  environment: ${ENV_ARGS[*]}"
