...
#
# Copied from applications/terminals/common/pin-windows.sh; the file is
# group-agnostic and the video group needs the same guarantees - pinned
# geometry, no window-manager key grabs, no toolbar clock in the capture.
#
...
if [[ -n "$MAIN_NAME" && "$WIDTH" == "0" ]]; then
    log "main window '${MAIN_NAME}' undecorated at 0,0; size left to the app"
