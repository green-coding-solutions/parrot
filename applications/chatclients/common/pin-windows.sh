...
#
# Copied unchanged from applications/emailclients/common/pin-windows.sh. The
# mechanism is not client-specific; the examples below are, and they are the
# email group's because NONE OF THE SIX CHAT CLIENTS HAS BEEN RUN YET. Each
# one's res_name has to be read off its own running window before its
# usage_scenario can pass a correct value - see the xprop recipe under
# MAIN_NAME. Passing a name that matches nothing is silent: fluxbox applies no
# rule, the window lands wherever it likes, and every click coordinate in the
# recording is off by an amount that changes per run.
#
# Expect the Electron pair to need MAIN_CLASS or MAIN_ROLE, for the reason
# BlueMail and Claws Mail did: Electron applications routinely map more than
# one window sharing a WM_CLASS.
#
